...
3. 8 个首批真实问数问题如何覆盖解析、规划、资产发现、指标解析和真实查数。
4. 真实货架可按测试需要调整，但目前只有广告节点下逻辑实体能取得最终数据库数据时，如何划分测试层级。
...
2. 真实货架没有业务 aliases；稳定口语、英文和缩写由业务概念模型维护。
...
分类目录应由 databp 进程内真实树生成一个 category-only 视图，过滤逻辑实体；具体是新增内部方法、复用已有遍历还是扩展现有方法，
由弱 Agent 检查真实 Java 工程后决定，不在本文编造方法名和包路径。
...
4. 合同第一轮不要求所有答案已知；未知内容由能访问真实工程和货架的弱 Agent调查并填写。
...
当前只确认：**只有广告节点下的逻辑数据实体能够完成最终数据库数据查询。**
...
8 个问题都必须通过 T1/T2；能否通过 T3 由弱 Agent调查后填写。T4 只能选最终映射到可查真实数据的广告逻辑实体的问题。
不得为了让智慧生活、小艺等问题通过 T4 而使用假数据。

问题 4“广告点击的 P95 时延”即使位于广告范围，也只有在真实 RealModel 存在正确 P95 指标且该指标 ID 可查数时才可通过 T4；
若只有平均时延，正确结果应是 `METRIC_NOT_FOUND` 或对应的受控状态，不能替代。

问题 6“智慧生活和广告的成功率对比”涉及跨范围对比。智慧生活一侧若没有可查真实数据，该问题仍可验证概念解析、两个 scope 和对比计划，
但不能伪造完整对比结果。
...
1. Java 弱 Agent 填写 §5 的 8 问语义真值表。
...
7. 记录 8 问分别能达到 T1/T2/T3/T4 的哪一级，不承诺未知能力。
...
3. 从现有 Phase A aliases 和 Phase B Metric Family 中只迁移 8 问需要的稳定知识。
4. 不复制 L2.3/L2.4 整树节点。
5. 编写 YAML 加载与静态校验测试；具体是复用现有 loader 还是新增 loader，由阶段 0 调查决定。
6. 用离线测试将 8 问人工真值编译为预期 interpretation 和 plan 快照。
...
- 8 问在云端 Agent 中达到各自允许的最高测试等级。
- 至少一个满足当前广告限制的用例完成 T4 真实查数。
...
```json
{
  ...
  "filterPhrases": ["<用户原话>"],
  "timeExpression": "<用户原话>",
  "queryMode": "<待 8 问合同确认枚举>"
}
```

是否支持多个业务对象、多个指标以及 comparison 的精确结构，由问题 6 的真实语义合同决定。
...
| 8 问对应的真实货架 ID 和路径 | Java 弱 Agent / 项目负责人 | 阶段 0 |
| 哪些问题可获得真实 RealModel 指标 | Java 弱 Agent | 阶段 0 |
| 广告范围内哪些指标 ID 可完成真实数据查询 | Java 弱 Agent | 阶段 0 |
| 问题 6 多业务对象 comparison 的输入合同 | 项目负责人 / Java 弱 Agent | 阶段 0 |
| “家庭组网”预期是稳定概念、动态节点口语还是未知词测试 | 项目负责人 / Java 弱 Agent | 阶段 0 |
...
|---|---|---|---|---|
| 0 | 真实盘点 + 8 问合同 | 不写生产代码 | 保持现状 | 所有未知项有记录 |
| 1 | 语义资源 schema + 校验 | 只做必要 loader/validator | 保持现状 | 资源能表达 8 问 |
| 2 | 动态分类目录 | 最小进程内目录与索引 | 保持现状 | 新 L2.3 无需 YAML |
| 3 | 计划生成接口 | 实现 resolver/mapping/plan，不执行数据 | 尚未切流 | 8 问 T1/T2 通过 |
...
1. 8 个固定问题都有经真实货架验证的语义合同和明确测试等级。
