...
2. 真实货架没有 `nameAliasCn/nameAliasEn`；业务 aliases 当前确实由 Phase A YAML 人工维护，不能假设可从货架自动同步。
...
迁移必须从可验证的问题合同开始，而不是先写通用框架。首批固定使用 8 个问题；其真实货架 ID、逻辑实体、正确语义和可执行性
由能够访问 Java 工程与真实货架的弱 Agent 补齐。由于目前只有广告节点下的逻辑数据实体能够完成最终真实查数，所有问题都可以做语义解析和计划测试，
但只有满足该真实数据前提的用例才能作为端到端真实数值验收；其他用例止于计划、资产发现或指标解析，不得用假数据伪装通过。
